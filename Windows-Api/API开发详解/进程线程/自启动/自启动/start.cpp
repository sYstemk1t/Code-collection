...
int autostart() {
	::GetModuleFileName(NULL, LPWSTR(tar), MAX);	//获取自身路径

	LONG ret1 = ::RegOpenKeyEx(HKEY_LOCAL_MACHINE, LPCWSTR(src), 0, KEY_ALL_ACCESS, &hkey);	//打开注册表
	if (ret1 != ERROR_SUCCESS)
		return 1;

	LONG ret2 = ::RegSetValueEx(hkey, LPCWSTR("test"), 0, REG_SZ, (const unsigned char*)tar, MAX);	//修改注册表
